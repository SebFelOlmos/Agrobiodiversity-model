...
def run_simulation(choice, variable): #Simulations!
    print('Lets go')
    ...
    if choice == '1':
        ...
        if variable  == '1':
                ...
                for j, iniciais  in enumerate(Emalphas):
                    ...
                    while contador < experiments and iterations < max_iterations: 
                            ...
                            for t in range(1, tiempo):
                                ...
                                for id_ud, ud in uds_copy.items():
                                    if ud.activa == True:
                                        ...
                                        ud.ter_filho()
                                        ud.buscar_ud(available, int(media), inherit)
                            ...
                            for t in range(1, tiempo):
                                ...
                                for id_ud, ud in uds_copy.items():
                                    if ud.activa == True:
                                        available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                        ud.ter_filho()
                                        ud.buscar_ud(available, int(media), varieties_list, 3)
                                        ud.incrementar_idade()
                                        ud.death_probability(prob_morte)
                                # Collecting data
                                vivas_tasa_crecimiento.append(len([ud for id, ud in uds_copy.items() if ud.activa]))
                        ...
                        if len(uds_copy) > 0:
                            ...
                            contador += 1
                        iterations += 1
                        print('succesful', contador,'iterations', iterations)
            ...
            plot = transversal_changes(EmM_datos, inherits, Emalphas, 'Endogamy', variable='Initial Households')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(EmM_datos, inherits, Emalphas, 'Endogamy', variable='Initial Households')
            ...
            plot = transversal_changes(EvM_datos, inherits, Evalphas, 'Endogamy', variable='Initial Varieties', limit = 14)
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            ...
            plot =transversal_changes(EcM_datos, inherits, Ecalphas, 'Endogamy', variable='Average number of children per household')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(EcM_datos, inherits, Ecalphas, 'Endogamy', variable='Average number of children per household')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")
        else:
            #sub folder
                            ...
                            for t in range(1, tiempo):
                                ...
                                for id_ud, ud in uds_copy.items():
                                    if ud.activa == True:
                                        available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                        ud.ter_filho()
                                        ud.buscar_ud(available, int(media), varieties_list, int(mvarieties))
                                        ud.incrementar_idade()
                                        ud.death_probability(prob_morte)
                                # Collecting data
                                vivas_tasa_crecimiento.append(len([ud for id, ud in uds_copy.items() if ud.activa]))
            ...
            plot = transversal_changes(EmvM_datos, inherits, Emvalphas, 'Endogamy', variable='Varieties per household')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(EmvM_datos, inherits, Emvalphas, 'Endogamy', variable='Varieties per household')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")
    elif choice == '2':
        ...
        if variable  == '1':
                            ...
                            for t in range(1, tiempo):
                                ...
                                for id_ud, ud in uds_copy.items():
                                    if ud.activa == True:
                                        available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                        ud.ter_filho()
                                        ud.buscar_ud(available, int(media), inherit, True, 0.2)
                                        ud.incrementar_idade()
                                        ud.death_probability(prob_morte)
                                # Collecting data
                                vivas_tasa_crecimiento.append(len([ud for id, ud in uds_copy.items() if ud.activa]))
                            ...
                            for t in range(1, tiempo):
                                ...
                                for id_ud, ud in uds_copy.items():
                                    if ud.activa == True:
                                        available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                        ud.ter_filho()
                                        ud.buscar_ud(available, int(media), varieties_list, True, 3, 0.2)
                                        ud.incrementar_idade()
                                        ud.death_probability(prob_morte)
                                # Collecting data
                                vivas_tasa_crecimiento.append(len([ud for id, ud in uds_copy.items() if ud.activa]))
            ...
            plot = transversal_changes(DmM_datos, inherits, Dmalphas, 'Dual Organization', variable='Initial Households')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(DmM_datos, inherits, Dmalphas, 'Dual Organization', variable='Initial Households')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")
        elif variable == '2':
            #sub folder
            folder_name = os.path.join(folder_name, 'Initial Varieties')
            ...
            plot = transversal_changes(DvM_datos, inherits, Dvalphas, 'Dual Organization', variable='Initial Varieties', limit = 14)
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            ...
            plot = transversal_changes(DcM_datos, inherits, Dcalphas, 'Dual Organization', variable='Average number of children per household')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(DcM_datos, inherits, Dcalphas, 'Dual Organization', variable='Average number of children per household')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")
        else:
            #sub folder
            folder_name = os.path.join(folder_name, 'Number of varieties for UD')
            ...
            for i, inherit in enumerate(inherits):
                for j, mvarieties  in enumerate(Dmvalphas):
                    ...
                    data_step_var_system = []
                    data_step_unique_system = []
                    print(f"Experiments with inheritance {inherit} Initial UDs {m} and varieties per UD {mvarieties}")
                    #Lists to store experiments' diversity data
                    data_endo_mean_ud = []
                    data_endo_com_level = []
                    data_endo_total_varieties = []
                    ...
                    while contador < experiments and iterations < max_iterations: 
                        ...
                        if inherit != 'False':
                            ...
                            for t in range(1, tiempo):
                                ...
                                if not uds_copy: #Extintion
                                    # Only 0
                                    #print(f"acabó en 0 el experimento {contador+1} con herencia {inherits[i]} con variedades {Dmvalphas[j]}")
                                    break         
                                # Cycle:
                                for id_ud, ud in uds_copy.items():
                                    if ud.activa == True:
                            ...
                            for t in range(1, tiempo):
                                ...
                                for id_ud, ud in uds_copy.items():
                                    if ud.activa == True:
                                        available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                        ud.ter_filho()
                                        ud.buscar_ud(available, int(media), varieties_list, True, int(mvarieties), 0.2)
                                        ud.incrementar_idade()
                                        ud.death_probability(prob_morte)
                                # Collecting data
                                vivas_tasa_crecimiento.append(len([ud for id, ud in uds_copy.items() if ud.activa]))
                    ...
                    print(f"""
            The total time of the configuration of inherit percentage {inherit}, 
            initial UDs {m} and varieties per household {mvarieties} is  {end_config - star_configuration}
                    """)
            # Folder
            ...
            plot = transversal_changes(DmvM_datos, inherits, Dmvalphas, 'Dual Organization', variable='Varieties per household')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(DmvM_datos, inherits, Dmvalphas, 'Dual Organization', variable='Varieties per household')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")
    else:
        ...
        if variable  == '1':
            ...
            plot = transversal_changes(GmM_datos, inherits, Gmalphas, 'Generalized', variable='Initial Households')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(GmM_datos, inherits, Gmalphas, 'Generalized', variable='Initial Households')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")
        elif variable == '2':
            #sub folder
            folder_name = os.path.join(folder_name, 'Initial Varieties')
            ...
            heat_std = os.path.join(folder_name, 'Generalized_Initial_Varieties.npy')
            DvM_datos = np.load(heat_std, allow_pickle=True)
            plot = transversal_changes(DvM_datos, inherits, Dvalphas, 'Generalized', variable='Initial Varieties', limit = 14)
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            ...
            for i, inherit in enumerate(inherits):
                for j, media  in enumerate(Gcalphas):
                    ...
                    print(f"""
            The total time of the configuration of inherit percentage {inherit} and mean {media} is  {end_config - star_configuration}
                    """)
            ...
            plot = transversal_changes(GcM_datos, inherits, Gcalphas, 'Generalized', variable='Average number of children per household')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(GcM_datos, inherits, Gcalphas, 'Generalized', variable='Average number of children per household')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")
        else:
            #sub folder
            folder_name = os.path.join(folder_name, 'Number of varieties for UD')
            ...
            heat_std = os.path.join(folder_name, 'Generalized_Initial_Mean_varieties.npy')
            DmvM_datos = np.load(heat_std, allow_pickle=True)
            plot = transversal_changes(DmvM_datos, inherits, Gmvalphas, 'Generalized', variable='Varieties per household')
            plot.savefig(os.path.join(folder_name, 'diversity_behavior.png'), dpi=300, bbox_inches="tight")
            plot = transversal_alive(DmvM_datos, inherits, Gmvalphas, 'Generalized', variable='Varieties per household')
            plot.savefig(os.path.join(folder_name, 'populational_behavior.png'), dpi=300, bbox_inches="tight")

# Functions for the information
def transversal_changes(matrix, inherits, alphas, kinship_system, variable='', limit = 10):
    # Plot
...
def transversal_alive(matrix, inherits, alphas, kinship_system, variable=''):
    ...
    plt.title(f'''
    {kinship_system}
    Final households for Different {variable}''')
    plt.legend(title="Inheritance")
    plt.grid(True)
    plt.ylim(0, 250) 
    return plt
...
def manual_config():
    ...
    while choice not in ['1', '2','3']:
        choice = input('''
    Hi! you want to execute a specific configuration for the second experiment! 
    ...
    Nice. Now, what variable you want to change over time?
        ...
        ''')
    run_simulation(choice, variable)
def all_config():
    kinship_systems = ['1', '2', '3']
    variables = ['1', '2', '3', '4']
    for choice in kinship_systems:
...
Select the mode you want to run:
    1 - Manual Configuration (Choose a specific configuration for the experiment)
    2 - Run All Configurations (Automatically run all simulations)
''')
if mode == '1':
    manual_config()
elif mode == '2':
