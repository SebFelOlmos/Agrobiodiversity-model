...
def simulation(choice):
    ...
    if choice == '1': #Endogamy
        ...
        for inherit in inheritance:
                    ...
                    for i in range (1, (t+1)):
                        ...
                        for id_ud, ud in uds_copy.items():
                            if ud.activa == True:
                                ...
                                ud.ter_filho()
                                ud.buscar_ud(available, media, inherit)
                    ...
                    for i in range (1, (t+1)):
                        ...
                        for id_ud, ud in uds_copy.items():
                            if ud.activa == True:
                                available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                ud.ter_filho()
                                ud.buscar_ud(available, media, varieties_list, 3)
                                ud.incrementar_idade()
                                ud.death_probability(prob_morte)
                        if i%view == 0:
                            var_ud = []
            ...
            print({extintion/(contador+extintion)})
                    ...
                    for i in range (1, (t+1)):
                        ...
                        for id_ud, ud in uds_copy.items():
                            if ud.activa == True:
                                available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                ud.ter_filho()
                                ud.buscar_ud(available, media, inherit, True, 0.2)
                                ud.incrementar_idade()
                                ud.death_probability(prob_morte)
                        if i%view == 0:
                            var_ud = []
                    ...
                    for i in range (1, (t+1)):
                        ...
                        for id_ud, ud in uds_copy.items():
                            if ud.activa == True:
                                available = {id_ud: ud for id_ud, ud in uds_copy.items() if ud.activa}
                                ud.ter_filho()
                                ud.buscar_ud(available, media, varieties_list, True, 3, 0.2)
                                ud.incrementar_idade()
                                ud.death_probability(prob_morte)
                        if i%view == 0:
                            var_ud = []
...
Do you want to see the diversity by every system? if you do not, you go directly to the general choice.
...
if all == 'y':
    ...
    plt.xlabel('Percentage of matrilineal inheritance ($\mathit{h}$)')
    plt.ylabel('Diversity at the community level')
    plt.ylim(0, 10)
    plt.title('Diversity at the community level for different kinship systems and $\mathit{h}$ values')
    plt.legend()
    plt.grid(True)
...
while option not in ['1','2', '3']:
    option = input('''
So, do you want to simulate one kinship system or all of them?
    1 - One kinship system
    2 - All kinship systems
    ''')
if option == '1':
    choice = ''
    ...
    print('Lets go with all of them :D')
    for n in ['1','2','3']:
        simulation(n)
