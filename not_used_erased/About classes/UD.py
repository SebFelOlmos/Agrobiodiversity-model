    ...
    def buscar_ud(self, media, matrilinear, locality): #metodo para buscar e reproducir.
        ...
        if filha_disponivel:
            ...
            if new_couple:
                ...
                novo_id = max(self.uds) + 1
                if locality == True:
                    novo_nodo = UD_endogamy(novo_id, media, self.uds[cassal_id].community)
                else:
                    novo_nodo = UD_endogamy(novo_id, media, self.community)
                novo_nodo.parentes[1] = self.id   # Familia materna [0]
                novo_nodo.parentes[0] = cassal_id # Familia paterna [1] 
...
class UD_dual_organization:
    ...
    def __init__(self, id_ud, media, community, clan, f_clan = 0, m_clan = 0):
        ...
        self.clan = clan
        self.uds[id_ud] = self
        self.female = [id_ud + 0.5, f_clan]
        self.male = [id_ud + 0.1, m_clan]
    ...
    def inherit(self, matrilinear): #Metodo da herança de sementes.
        ...
        if matrilinear == 0:
            ...
            vari_mae = random.sample(vm, heredar_mae)
            vari_pai = random.sample(vp, heredar_pai) 
            # Extend the varieties list with the combined samples
            self.varieties.extend(vari_mae + vari_pai)

        
class UD_generalized:
    ## CLase UD_generalized
    ## IMPORTANT: The class "select_new_ud" is the one that applies the weight according to clan and community. 
    ## The weight can be changed to represent a spectrum of exogamy rules. It means how much importance do you give
    ## To the clan or community differences
    ## A restrcited system defines the clan of your possible partner. Function restrict helps here.
    ...
    def buscar_ud(self, media, matrilinear, locality, inheritance): #To look and reproduce
        ...
        #Function to determine the restricted exchange. Only 3 clans, True if b is is the allowed clan for a, else False
        def restrict(a,b):
        ...
        if filha_disponivel:
            ...
            if new_couple:
                ...
                if locality == True:
                    if inheritance == True:
                        novo_nodo = UD_generalized(novo_id, media, self.community, self.clan, self.clan, self.uds[cassal_id].clan) # Matrilocal and matrilineal
                    else:
                        novo_nodo = UD_generalized(novo_id, media, self.community, self.uds[cassal_id].clan, self.clan, self.uds[cassal_id].clan) # Matrilocal and patrilineal
                else:
                    if inheritance == True:
                        novo_nodo = UD_generalized(novo_id, media, self.uds[cassal_id].community, self.clan, self.clan, self.uds[cassal_id].clan) # Patrilocal and matrilineal
                    else:
                        novo_nodo = UD_generalized(novo_id, media, self.uds[cassal_id].community, self.uds[cassal_id].clan, self.clan, self.uds[cassal_id].clan) # Patrilocal and patrilineal
                novo_nodo.parentes[1] = self.id   # Familia materna [0]
                novo_nodo.parentes[0] = cassal_id # Familia paterna [1] 
