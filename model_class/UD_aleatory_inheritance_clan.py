    ...
    def buscar_ud(self, uds_systema, media, variedades, k=3): #metodo para buscar e reproducir.     
        ...
        if filha_disponivel:
            uds_disponiveis = [id_ud for id_ud, ud in uds_systema.items() if
                    ...
                    abs(filho_e_idade(ud,1)[2] - idade_filha) <= 13 ] #Que a diferença entre as idades não seja maior a 15
            
            if uds_disponiveis:
                cassal_id = random.choice(uds_disponiveis)
                ...
                novo_nodo = UD_endogamy_al_inh(novo_id, media)
    ...
    def buscar_ud(self, uds_systema, media, variedades, lineality, k=3, percentage_incest = 0.5): #metodo para buscar e reproducir.
        ...
        if filha_disponivel:
            ...
            if new_couple:
                ...
                novo_id = max(self.uds) + 1
                if lineality == True:
                    novo_nodo = UD_dual_organization_al_inh(novo_id, media, self.uds[cassal_id].clan)
                else:
                    novo_nodo = UD_dual_organization_al_inh(novo_id, media, self.clan)
...
class UD_generalized_al_inh:
    ...
    ## Class with generalized. In other words, UDs in this model belong to three different clans and the only criteria is 
    # the incest rule in relation to the clan
    #Dict de todas as UDs
    uds = {}
    ...
    def buscar_ud(self, uds_systema, media, variedades, lineality, k=3, percentage_incest = 0.5): #metodo para buscar e reproducir.
        ...
        if filha_disponivel:
            ...
            if new_couple:
                ...
                novo_id = max(self.uds) + 1
                if lineality == True:
                    novo_nodo = UD_generalized_al_inh(novo_id, media, self.uds[cassal_id].clan)
                else:
                    novo_nodo = UD_generalized_al_inh(novo_id, media, self.clan)
                novo_nodo.parentes[1] = self.id   # Familia materna [0]
                novo_nodo.parentes[0] = cassal_id # Familia paterna [1] 
